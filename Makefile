...
TARG.Name:=runtime-provider-qingcloud
TRAG.Gopkg:=openpitrix.io/runtime-provider-qingcloud
...
.PHONY: build
build: fmt ## Build all runtime-provider-qingcloud images
	mkdir -p ./tmp/bin
...
.PHONY: compose-up
compose-up: ## Launch runtime-provider-qingcloud in docker compose
	docker-compose up -d
...
build-image-%: ## build docker image
	@if [ "$*" = "latest" ];then \
	docker build -t openpitrix/runtime-provider-qingcloud:latest .; \
	elif [ "`echo "$*" | grep -E "^v[0-9]+\.[0-9]+\.[0-9]+"`" != "" ];then \
	docker build -t openpitrix/runtime-provider-qingcloud:$* .; \
	fi

push-image-%: ## push docker image
	@if [ "$*" = "latest" ];then \
	docker push openpitrix/runtime-provider-qingcloud:latest; \
	elif [ "`echo "$*" | grep -E "^v[0-9]+\.[0-9]+\.[0-9]+"`" != "" ];then \
	docker push openpitrix/runtime-provider-qingcloud:$*; \
	fi